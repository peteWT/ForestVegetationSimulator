...
C----------
     ...
     1              CRATIO,BARK,ITRNC,VMAX,BFMAX,
     1              RCULL,DECAY,WDSTMS,BIODRYIN,
     1              LIVEDEAD,CTKFLG,BTKFLG,IT)
C-----------
...
C        IF(IREGN.EQ.8 .AND. VOLEQ(4:6).EQ.'CLK')THEN
C          WRITE(BFTOP,'(I1)')NINT(BFTOPD(ISPC))
C          VOLEQ(3:3)=BFTOP
C        ENDIF
        DO IZERO=1,15
        TVOL(IZERO)=0.
