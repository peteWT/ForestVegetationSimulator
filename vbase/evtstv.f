...
C
C     120 BSCUFT    BEFORE THIN CUBIC SAW VOLUME CUFT
C
...
C
C     220 ASCUFT    AFTER THIN THIN SAW LOG MERCH CUFT
C     221 RSCUFT    REMOVED SAW LOG MERCH CUFT
C
C     BIOMASS AND CARBON MEASURES ADDED SEPTEMBER 2024
...
C
      IF(ICYC.EQ.1) THEN
        IF(TSTV1(2).GT.0.)THEN
          SELECT CASE (VARACD)
C            CASE ('CS','LS','NE','SN','ON')
C              BCYMAI(ICYC)=IOSUM(4,ICYC)/TSTV1(2)
            CASE DEFAULT
