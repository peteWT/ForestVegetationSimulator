...
C     SAMPLING WEIGHT AND STAND LABEL SET (GROUPS) FOR STAND HAS BEEN
C     SET. FVS_CASES OUTPUT DATABASE RECORD WILL BE UPDATED IF IT EXISTS.
...
C      IF(LNOTBK(2).AND.ARRAY(2).GT.0.) IFINT = IFIX(ARRAY(2))
C      FINT = FLOAT(IFINT)
      IF(LNOTBK(2).AND.ARRAY(2).GT.0.) FINT = ARRAY(2)

      IF(LNOTBK(3)) IHTG = IFIX(ARRAY(3))
C      IF(LNOTBK(4).AND.ARRAY(4).GT.0.) IFINTH = IFIX(ARRAY(4))
C      FINTH = FLOAT(IFINTH)
      IF(LNOTBK(4).AND.ARRAY(4).GT.0.) FINTH = ARRAY(4)

c      IF(LNOTBK(5).AND.ARRAY(5).GT.0.) FINTM=ARRAY(5)
c      I=IFIX(FINTM)

      IF(LNOTBK(5).AND.ARRAY(5).GT.0.) FINTM=ARRAY(5)

C      IF(LKECHO)WRITE(JOSTND,2310) KEYWRD,IDG,IFINT,IHTG,IFINTH,I
C 2310 FORMAT (/A8,'   DIAMETER GROWTH DATA TYPE CODE= ',I1,
C     >        ', ',I3,' YEAR MEASUREMENT PERIOD;' /
C     >        T12,'HEIGHT GROWTH DATA TYPE CODE  = ',I1,
C     >        ', ',I3,' YEAR MEASUREMENT PERIOD;'/
C     >        T12,'MORTALITY MEASUREMENT PERIOD  =',I2)
...
C     PREVIOUS VERSIONS HAD A FIXED TOP HEIGHT FOR REGION 8, 
C     INDICATED BY A 'FLAG' OF ITOPD8=1, THIS IS NO LONGER 
C     NECESSARY AND ALL REFERENCES TO ITOPD8 HAVE BEEN REMOVED
C
C     SPECIES CODE PROCESSING.
C
      CALL SPDECD (2,IS,NSP(1,1),JOSTND,IRECNT,KEYWRD,
...
C
C  ==========  OPTION NUMBER 147: FIAVBC  ==========================FIAVBC
C
14700 CONTINUE
