...
C SCFMIN  = MINIMUM HARVEST IN SAWLOG CUBIC FOOT
C
...
C   SCCUT= TOTAL SAWLOG CUBIC FOOT CUT
C
...
C           ARE BEING REMOVED DURING CURRENT PRESCRIPTION THIN. A SPECIAL
C           VALUE OF -1 MEANS THAT WK6 IS PRELOADED WITH THE PROPORTION OF 
C           EACH TREE TO BE REMOVED. 
...
C     WK6 = STORES REMOVED MERCH BF VOLUME. UPON THE SUBROUTINE ENTRY
C           WK6 CAN BE PRELOADED WITH THE PROPORTION OF EACH TREE THAT
C           WILL BE REMOVED WHEN THINPREC IS USED WITH KUTNOW SET TO -1
C
...
C    OSCREM = REMOVED CUBIC SAW FOOT VOLUME
C
...
C        THIS CODE WAS COMMENTED OUT BECAUSE SOME EVENT MONITOR
C        VARIABLES WERE NOT GETTING CALCULATED WHEN ALL TREES WERE DEAD
C        SPECIFIC CASE WAS WILDFIRE CAUSING 100% MORTALITY AND THEN
C        A COMPUTED SALVAGE VOLUME ON SUBSEQUENT CYCLE, 2 YEARS POST FIRE.
C        LANCE DAVID 03/05/2020
C
C        IF (ITRN.LE.0)THEN
CC----------
CC  CHECK FOR REQUEST OF SALVAGE ACTION FOR THE SPECIAL CASE WHERE ALL
CC  TREES IN THE STAND WERE KILLED
CC----------
C          CALL FMSALV (IY(ICYC),SALVTPA)
C          IF ( DEBUG ) THEN
C          WRITE (JOSTND,9013) SALVTPA
C 9013     FORMAT(' ','IN CUTS, FOLLOWING CALL TO FMSALV',
C     &    ' FOR SPECIAL CASE WHERE ALL TREES IN STAND ARE KILLED:',
C     &    ' SALVTPA=',F7.2)
C          GO TO 1900
C          ENDIF
C        ENDIF
...
C  AT THE 7TH DECIMALL PLACE. THIS CAUSED THE XLEFT VARIABLE TO TO HAVE
...
C----------
 ...
 1800 CONTINUE
C----------
C  COMPUTE PERCENTILE POINTS IN THE DISTRIBUTION OF REMOVED CARBON AND BIOMASS.
C----------
      IF(LFIANVB) THEN
